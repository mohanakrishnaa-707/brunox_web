...
// Simple Chat Contract ABI for local deployment
...
export const GanacheProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const connectToGanache = async () => {
    try {
      ...
      // Fallback: simulate local blockchain
  ...
  const createSimulatedBlockchain = () => {
    // Create a simulated blockchain environment
    const simulatedProvider = new ethers.JsonRpcProvider('http://localhost:8545');
    ...
    const mockContract = {
      sendMessage: async (to: string, message: string) => {
        ...
        console.log('Mock message sent:', { to, message, hash });
        return hash;
      },
    ...
    console.log('Simulated blockchain created');
  };

  const sendMessage = async (to: string, message: string): Promise<string> => {
